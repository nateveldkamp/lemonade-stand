...
- **Onboarding front door (built 2026-06-04, pushed to `main`).** Learners no longer get a zip + PDF. Instead they paste the repo link / open the **GitHub Pages site** in any AI chat (phone or computer) and the AI runs a guided opening. Entry artifacts at repo root: `index.html` (landing), `about.html` (why a lemonade stand / "play the whole game" — David Perkins / business+AI-together), `start.txt` (the canonical AI guide the prompt points to), `llms.txt`, `.nojekyll`; plus `game/play-by-chat.md` (Python-free game rules). The root `README.md` is also dual-audience (human intro + AI-guide instructions). Browser/mobile-first; learners graduate to working out of a cloned repo (Claude Code / Codex / Cursor — their choice) much later. **Before touching onboarding, read `memory/onboarding-front-door.md` (has status + pending: enable GitHub Pages, run the phone test pass).** Do NOT steer learners to a specific AI provider; steer toward graduating to a repo so they stop copy-pasting.
